...
async def demo_sequential():
    ...
    results = await orch.execute_sequential([
        ...
        Task(name="读取README", coroutine=read_readme,
             kwargs={"owner": "x", "repo": "y"}, depends_on=["搜索仓库"]),
        Task(name="代码分析", coroutine=analyze_code,
             kwargs={"code": "..."}, depends_on=["读取README"]),
        Task(name="生成报告", coroutine=generate_report,
             kwargs={"data": {}}, depends_on=["搜索仓库", "代码分析"]),
    ])
...
async def demo_parallel():
    ...
    results = await orch.execute_parallel([
        ...
        Task(name="搜索文档", coroutine=unstable_api,
             kwargs={"url": "https://docs.example.com"}),
    ])
    _print_summary(results)
...
async def demo_conditional():
    ...
    def decide(results: dict[str, TaskResult]) -> list[Task]:
        ...
        if r and r.status == TaskStatus.SUCCESS and r.data.get("score", 0) < 85:
            print("  → 评分 < 85，触发优化建议")
            return [Task(name="优化建议", coroutine=unstable_api,
...
async def demo_retry_fallback():
    ...
    results = await orch.execute_parallel([
        Task(
            name="不稳定API", coroutine=unstable_api,
            kwargs={"url": "https://unstable.example.com"},
            max_retries=2, timeout_seconds=1.0,
...
def _print_summary(results: dict[str, TaskResult]):
    ...
    for name, r in results.items():
        icon = {TaskStatus.SUCCESS: "OK", TaskStatus.FAILED: "FAIL",
                TaskStatus.FALLBACK: "降级", TaskStatus.TIMEOUT: "超时",
                TaskStatus.SKIPPED: "跳过"}.get(r.status, "?")
